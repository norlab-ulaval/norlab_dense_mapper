...
norlab_dense_mapper::DenseMap::DenseMap(const float& minDistNewPoint, const float& sensorMaxRange, const float& priorDynamic, const float& thresholdDynamic,
...
void norlab_dense_mapper::DenseMap::loadCells(int startRow, int endRow, int startColumn, int endColumn, int startAisle, int endAisle)
{
	if(!is3D)
	{
		startAisle = 0;
...
void norlab_dense_mapper::DenseMap::unloadCells(int startRow, int endRow, int startColumn, int endColumn, int startAisle, int endAisle)
{
	if(!is3D)
	{
		startAisle = 0;
...
int norlab_dense_mapper::DenseMap::toInferiorGridCoordinate(const float& worldCoordinate, const float& range) const
{
	return std::ceil(((worldCoordinate - range) / CELL_SIZE) - 1.0);
}

int norlab_dense_mapper::DenseMap::toSuperiorGridCoordinate(const float& worldCoordinate, const float& range) const
{
	return std::floor((worldCoordinate + range) / CELL_SIZE);
}

norlab_dense_mapper::DenseMap::PM::DataPoints norlab_dense_mapper::DenseMap::retrievePointsFurtherThanMinDistNewPoint(const PM::DataPoints& input,
...
void norlab_dense_mapper::DenseMap::computeProbabilityOfPointsBeingDynamic(const PM::DataPoints& input, PM::DataPoints& currentLocalPointCloud,
																	const PM::TransformationParameters& pose) const
{
	typedef Nabo::NearestNeighbourSearch<float> NNS;
...
void norlab_dense_mapper::DenseMap::convertToSphericalCoordinates(const PM::DataPoints& points, PM::Matrix& radii, PM::Matrix& angles) const
{
...
void norlab_dense_mapper::DenseMap::updateLocalPointCloud(PM::DataPoints input, PM::TransformationParameters pose, PM::DataPointsFilters postFilters)
{
