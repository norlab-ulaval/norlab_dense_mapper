...
norlab_dense_mapper::DenseMap::DenseMap(const float& minDistNewPoint,
    ...
    updateThreadLooping(true)
{
    ...
    if (isOnline)
    {
        updateThread = std::thread(&DenseMap::updateThreadFunction, this);
    }
}

norlab_dense_mapper::DenseMap::~DenseMap()
{
    if (isOnline)
    {
...
void norlab_dense_mapper::DenseMap::updateThreadFunction()
{
...
void norlab_dense_mapper::DenseMap::scheduleUpdate(const Update& update)
{
    if (isOnline)
    {
        updateListLock.lock();
...
void norlab_dense_mapper::DenseMap::applyUpdate(const Update& update)
{
    if (update.load)
    {
        loadCells(update.startRow,
                  update.endRow,
                  update.startColumn,
                  update.endColumn,
                  update.startAisle,
...
void norlab_dense_mapper::DenseMap::loadCells(int startRow,
                                              int endRow,
                                              int startColumn,
                                              int endColumn,
                                              int startAisle,
                                              int endAisle)
{
    ...
    for (int i = startRow; i <= endRow; ++i)
    {
        for (int j = startColumn; j <= endColumn; ++j)
        {
            for (int k = startAisle; k <= endAisle; ++k)
            {
                ...
                PM::DataPoints cell = cellManager->retrieveCell(
    ...
    for (int i = startRow; i <= endRow; ++i)
    {
        for (int j = startColumn; j <= endColumn; ++j)
        {
            for (int k = startAisle; k <= endAisle; ++k)
            {
                loadedCellIds.insert(std::to_string(i) + "_" + std::to_string(j) + "_" +
                                     std::to_string(k));
            }
        }
    }
...
void norlab_dense_mapper::DenseMap::unloadCells(int startRow,
                                                int endRow,
                                                int startColumn,
                                                int endColumn,
                                                int startAisle,
                                                int endAisle)
{
    ...
    {
        if (localPointCloud.features(0, i) >= startX && localPointCloud.features(0, i) < endX &&
            localPointCloud.features(1, i) >= startY && localPointCloud.features(1, i) < endY &&
            localPointCloud.features(2, i) >= startZ && localPointCloud.features(2, i) < endZ)
        {
    ...
    if (!loadedCellIds.empty())
    {
        for (int i = startRow; i <= endRow; ++i)
        {
            for (int j = startColumn; j <= endColumn; ++j)
            {
                for (int k = startAisle; k <= endAisle; ++k)
                {
                    loadedCellIds.erase(std::to_string(i) + "_" + std::to_string(j) + "_" +
                                        std::to_string(k));
                }
            }
        }
    ...
    for (int i = 0; i < oldChunk.getNbPoints(); i++)
    {
        ...
        std::string cellId =
            std::to_string(row) + "_" + std::to_string(column) + "_" + std::to_string(aisle);
...
float norlab_dense_mapper::DenseMap::toInferiorWorldCoordinate(const int& gridCoordinate) const
{
    return gridCoordinate * CELL_SIZE;
}

float norlab_dense_mapper::DenseMap::toSuperiorWorldCoordinate(const int& gridCoordinate) const
{
    return (gridCoordinate + 1) * CELL_SIZE;
}

int norlab_dense_mapper::DenseMap::getMinGridCoordinate() const
{
    return std::numeric_limits<int>::lowest();
}

int norlab_dense_mapper::DenseMap::getMaxGridCoordinate() const
{
    return std::numeric_limits<int>::max() - 1;
}

int norlab_dense_mapper::DenseMap::toGridCoordinate(const float& worldCoordinate) const
{
    return std::floor(worldCoordinate / CELL_SIZE);
}

int norlab_dense_mapper::DenseMap::toInferiorGridCoordinate(const float& worldCoordinate,
                                                            const float& range) const
{
    return std::ceil(((worldCoordinate - range) / CELL_SIZE) - 1.0);
}

int norlab_dense_mapper::DenseMap::toSuperiorGridCoordinate(const float& worldCoordinate,
                                                            const float& range) const
{
    return std::floor((worldCoordinate + range) / CELL_SIZE);
}

norlab_dense_mapper::DenseMap::PM::DataPoints
norlab_dense_mapper::DenseMap::retrievePointsFurtherThanMinDistNewPoint(
    ...
    const PM::TransformationParameters& pose) const
{
    ...
    PM::Matches matches(PM::Matches::Dists(1, input.getNbPoints()),
...
void norlab_dense_mapper::DenseMap::computeProbabilityOfPointsBeingDynamic(
    const PM::DataPoints& input,
    ...
    const PM::TransformationParameters& pose) const
{
    ...
    convertToSphericalCoordinates(currentLocalPointCloudInSensorFrame,
                                  currentLocalPointCloudInSensorFrameRadii,
                                  currentLocalPointCloudInSensorFrameAngles);
    ...
    nns->knn(currentLocalPointCloudInSensorFrameAngles,
             ids,
             dists,
             1,
             0,
    ...
    PM::DataPoints::View viewOnProbabilityDynamic =
    ...
    for (int i = 0; i < currentLocalPointCloudInSensorFrame.getNbPoints(); i++)
    {
        if (dists(i) != std::numeric_limits<float>::infinity())
        {
            ...
            const Eigen::VectorXf inputPoint = inputInSensorFrame.features.col(inputPointId)
                                                   .head(inputInSensorFrame.getEuclideanDim());
            const Eigen::VectorXf localPointCloudPoint =
                currentLocalPointCloudInSensorFrame.features.col(i).head(
                    currentLocalPointCloudInSensorFrame.getEuclideanDim());
            ...
            const float w_v =
                eps + (1. - eps) *
                          fabs(localPointCloudPointNormal.dot(localPointCloudPoint.normalized()));
            ...
            if ((inputPoint.norm() + epsilonD + d_max) >= localPointCloudPoint.norm())
            {
                ...
                {
                    probDynamic =
                        c1 * lastDyn + c2 * w_d2 * ((1 - alpha) * (1 - lastDyn) + beta * lastDyn);
                    probStatic = c1 * (1 - lastDyn) +
                                 c2 * w_p2 * (alpha * (1 - lastDyn) + (1 - beta) * lastDyn);
                }
                else
                {
                    probDynamic = 1 - eps;
                ...
                viewOnProbabilityDynamic(0, localPointCloudPointId) =
                    probDynamic / (probDynamic + probStatic);
            }
        }
    }
}

void norlab_dense_mapper::DenseMap::convertToSphericalCoordinates(const PM::DataPoints& points,
                                                                  PM::Matrix& radii,
...
void norlab_dense_mapper::DenseMap::updatePose(const PM::TransformationParameters& pose)
{
    ...
    if (firstPoseUpdate.load())
    {
        ...
        unloadCells(getMinGridCoordinate(),
                    getMaxGridCoordinate(),
                    getMinGridCoordinate(),
                    getMaxGridCoordinate(),
                    getMinGridCoordinate(),
                    getMaxGridCoordinate());
        loadCells(inferiorRowLastUpdateIndex - BUFFER_SIZE,
                  superiorRowLastUpdateIndex + BUFFER_SIZE,
                  inferiorColumnLastUpdateIndex - BUFFER_SIZE,
                  superiorColumnLastUpdateIndex + BUFFER_SIZE,
                  inferiorAisleLastUpdateIndex - BUFFER_SIZE,
                  superiorAisleLastUpdateIndex + BUFFER_SIZE);
    ...
    else
    {
        ...
        if (std::abs(toInferiorGridCoordinate(pose(0, positionColumn), sensorMaxRange) -
                     inferiorRowLastUpdateIndex) >= 2)
        {
            // move back
            if (toInferiorGridCoordinate(pose(0, positionColumn), sensorMaxRange) <
                inferiorRowLastUpdateIndex)
            {
                int nbRows = inferiorRowLastUpdateIndex -
                             toInferiorGridCoordinate(pose(0, positionColumn), sensorMaxRange);
                int startRow =
                    toInferiorGridCoordinate(pose(0, positionColumn), sensorMaxRange) - 2;
                int endRow = toInferiorGridCoordinate(pose(0, positionColumn), sensorMaxRange) -
                             BUFFER_SIZE + nbRows - 1;
            ...
            if (toInferiorGridCoordinate(pose(0, positionColumn), sensorMaxRange) >
                inferiorRowLastUpdateIndex)
            {
                int nbRows = toInferiorGridCoordinate(pose(0, positionColumn), sensorMaxRange) -
                             inferiorRowLastUpdateIndex;
        ...
        if (std::abs(toSuperiorGridCoordinate(pose(0, positionColumn), sensorMaxRange) -
                     superiorRowLastUpdateIndex) >= 2)
        {
            // move back
            if (toSuperiorGridCoordinate(pose(0, positionColumn), sensorMaxRange) <
                superiorRowLastUpdateIndex)
            {
                int nbRows = superiorRowLastUpdateIndex -
                             toSuperiorGridCoordinate(pose(0, positionColumn), sensorMaxRange);
            ...
            if (toSuperiorGridCoordinate(pose(0, positionColumn), sensorMaxRange) >
                superiorRowLastUpdateIndex)
            {
                int nbRows = toSuperiorGridCoordinate(pose(0, positionColumn), sensorMaxRange) -
                             superiorRowLastUpdateIndex;
                int startRow = toSuperiorGridCoordinate(pose(0, positionColumn), sensorMaxRange) +
                               BUFFER_SIZE - nbRows + 1;
                int endRow =
                    toSuperiorGridCoordinate(pose(0, positionColumn), sensorMaxRange) + BUFFER_SIZE;
                int startColumn = inferiorColumnLastUpdateIndex - BUFFER_SIZE;
                int endColumn = superiorColumnLastUpdateIndex + BUFFER_SIZE;
                int startAisle = inferiorAisleLastUpdateIndex - BUFFER_SIZE;
                int endAisle = superiorAisleLastUpdateIndex + BUFFER_SIZE;
        ...
        if (std::abs(toInferiorGridCoordinate(pose(1, positionColumn), sensorMaxRange) -
                     inferiorColumnLastUpdateIndex) >= 2)
        {
            // move right
                ...
                inferiorColumnLastUpdateIndex)
            {
                int startRow = inferiorRowLastUpdateIndex - BUFFER_SIZE;
                int endRow = superiorRowLastUpdateIndex + BUFFER_SIZE;
                int nbColumns = inferiorColumnLastUpdateIndex -
                                toInferiorGridCoordinate(pose(1, positionColumn), sensorMaxRange);
                int startColumn =
                    toInferiorGridCoordinate(pose(1, positionColumn), sensorMaxRange) - BUFFER_SIZE;
                int endColumn = toInferiorGridCoordinate(pose(1, positionColumn), sensorMaxRange) -
                                BUFFER_SIZE + nbColumns - 1;
                int startAisle = inferiorAisleLastUpdateIndex - BUFFER_SIZE;
                int endAisle = superiorAisleLastUpdateIndex + BUFFER_SIZE;
                scheduleUpdate(
                    Update{startRow, endRow, startColumn, endColumn, startAisle, endAisle, true});
                ...
                inferiorColumnLastUpdateIndex)
            {
                int startRow = inferiorRowLastUpdateIndex - BUFFER_SIZE;
                int endRow = superiorRowLastUpdateIndex + BUFFER_SIZE;
                int nbColumns = toInferiorGridCoordinate(pose(1, positionColumn), sensorMaxRange) -
                                inferiorColumnLastUpdateIndex;
                int startColumn = inferiorColumnLastUpdateIndex - BUFFER_SIZE;
        ...
        if (std::abs(toSuperiorGridCoordinate(pose(1, positionColumn), sensorMaxRange) -
                     superiorColumnLastUpdateIndex) >= 2)
        {
            // move right
                ...
                superiorColumnLastUpdateIndex)
            {
                int startRow = inferiorRowLastUpdateIndex - BUFFER_SIZE;
                int endRow = superiorRowLastUpdateIndex + BUFFER_SIZE;
                int nbColumns = superiorColumnLastUpdateIndex -
                                toSuperiorGridCoordinate(pose(1, positionColumn), sensorMaxRange);
                ...
                superiorColumnLastUpdateIndex)
            {
                int startRow = inferiorRowLastUpdateIndex - BUFFER_SIZE;
                int endRow = superiorRowLastUpdateIndex + BUFFER_SIZE;
                int nbColumns = toSuperiorGridCoordinate(pose(1, positionColumn), sensorMaxRange) -
                                superiorColumnLastUpdateIndex;
                int startColumn =
                    toSuperiorGridCoordinate(pose(1, positionColumn), sensorMaxRange) +
                    BUFFER_SIZE - nbColumns + 1;
                int endColumn =
                    toSuperiorGridCoordinate(pose(1, positionColumn), sensorMaxRange) + BUFFER_SIZE;
                int startAisle = inferiorAisleLastUpdateIndex - BUFFER_SIZE;
                int endAisle = superiorAisleLastUpdateIndex + BUFFER_SIZE;
                scheduleUpdate(
                    Update{startRow, endRow, startColumn, endColumn, startAisle, endAisle, true});
        ...
        {
            // update cells below
            if (std::abs(toInferiorGridCoordinate(pose(2, positionColumn), sensorMaxRange) -
                         inferiorAisleLastUpdateIndex) >= 2)
            {
                // move down
            ...
            // update cells above
            if (std::abs(toSuperiorGridCoordinate(pose(2, positionColumn), sensorMaxRange) -
                         superiorAisleLastUpdateIndex) >= 2)
            {
                // move down
...
norlab_dense_mapper::DenseMap::PM::DataPoints norlab_dense_mapper::DenseMap::getLocalPointCloud()
{
    std::lock_guard<std::mutex> lock(localPointCloudLock);
    return localPointCloud;
}

void norlab_dense_mapper::DenseMap::updateLocalPointCloud(PM::DataPoints input,
                                                          PM::TransformationParameters pose,
                                                          PM::DataPointsFilters postFilters)
{
    if (computeProbDynamic)
    {
        input.addDescriptor("probabilityDynamic",
                            PM::Matrix::Constant(1, input.features.cols(), priorDynamic));
    }

    if(!input.descriptorExists("initialPosition"))
    {
...
bool norlab_dense_mapper::DenseMap::getNewLocalPointCloud(PM::DataPoints& localPointCloudOut)
{
...
norlab_dense_mapper::DenseMap::PM::DataPoints norlab_dense_mapper::DenseMap::getGlobalPointCloud()
{
    localPointCloudLock.lock();
...
bool norlab_dense_mapper::DenseMap::isLocalPointCloudEmpty() const
{
